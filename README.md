...
### 5. Прогнозирование типа личности (`Predict_human_psycotype`)
- **Задача:** Бинарная классификация. Предсказание психотипа по поведению и некоторым характеристикам человека.
- **Описание:** Классическая задача машинного обучения на табличных данных. Сделал глубокий анализ данных EDA и добавил несколько новых признаков для повышения качества классификации.
- **Методология:**
  - `Создать новые признаки`
  - `Применить Стандартизацию (StandartScaler)`
  - `Закодировать категориальные признаки (OrdinalEncoder)`
  - `Заполнить пропуски (kNN Imputer)`
  - `LogisticRegression`
  - `LightGBM`
  - `Подобрать гиперпараметры с помощью GridSearch`
- **[Посмотреть код](Predict_human_psycotype/model/)**
